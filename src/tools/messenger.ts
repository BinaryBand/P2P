...
export function quickHash(input: string): Uint8Array {
  const hash: Uint8Array = blake2b(input, { dkLen: 32 });
  return hash;
}
...
export function orderPeers(query: string, candidates: string[]): PeerDistancePair[] {
  const key: Uint8Array = quickHash(query);
  const distances: PeerDistancePair[] = [];
  for (const peer of new Set(candidates)) {
    const peerCode: Uint8Array = quickHash(peer);
